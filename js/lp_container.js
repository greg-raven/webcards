...
function hideMenuOnCorT(event) {
  ...
  if (
    !document.getElementById("ddmenuspan").contains(event.target) &&
    window.getComputedStyle(document.getElementById("dDMButton"), null)
      .visibility == "visible"
  ) {
  ...
  if (scViews.length > 0) {
    ...
    for (var i = 0; i < scViews.length; i++) {
      if (
        !showSCs[i].contains(event.target) &&
        scViews[i].style.display == "block" &&
        !scViews[i].contains(event.target)
      ) {
...
function showDDMenu() {
  if (!window.matchMedia("all and (min-width: 769px)").matches) {
    if (
      document.getElementById("menu").getElementsByTagName("ul")[0].style
        .width == ""
    ) {
      //set width of the menu ul element so that it shows
      document
        .getElementById("menu")
        .getElementsByTagName("ul")[0].style.width = "200px";
    } else {
      //clear menu ul element width so it hides
      document
        .getElementById("menu")
        .getElementsByTagName("ul")[0].style.width = "";
    }
  }
}
...
function searchArray(string, data) {
  ...
  if (string == "") {
    ...
    string = string
      ...
      .replace(/,/g, "")
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, ""); //make string uppercase, remove commas, remove diacritics (normalize('NFD').replace(/[\u0300-\u036f]/g, ""))
    for (var entryIterator = 0; entryIterator < data.length; entryIterator++) {
      ...
      if (!isNaN(string)) {
        ...
        for (
          var strElementIterator = 0;
          strElementIterator < stringArray.length;
          strElementIterator++
        ) {
          var elementMatches = 0;

          for (
            var keyElementIterator = 0;
            keyElementIterator < keyArray.length;
            keyElementIterator++
          ) {
            if (
              keyArray[keyElementIterator].startsWith(
                stringArray[strElementIterator],
              )
            ) {
              elementMatches++;
      ...
      for (var iterator = 0; iterator < results.length; iterator++) {
        ...
        if (anchorID != null) {
          ...
          if (pageType == "h" || pageType == "d") {
            url +=
              (alternatePID == null ? thisResult["i"] : alternatePID) + "F";
            /* url+= thisResult["i"] + "F"; */
          }
          url += anchorID;
        }
