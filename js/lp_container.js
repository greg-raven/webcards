...
window.addEventListener('click', function (clickEvent) {
    hideMenuOnCorT(clickEvent);
}); //click event
window.addEventListener('touchstart', function (clickEvent) {
    hideMenuOnCorT(clickEvent);
}); //touch event
...
function hideMenuOnCorT(event) { //hide menu on click or touch outside of menu
    if (!(document.getElementById("ddmenuspan").contains(event.target)) && window.getComputedStyle(document.getElementById("dDMButton"), null).visibility == "visible") { //clear styles set in JS, forcing fallback to CSS style file
        document.getElementById("menu").getElementsByTagName("ul")[0].style.width = "";
    }

    scViews = document.getElementsByClassName("scView");

    if (scViews.length > 0) { //don't excute any further code if there are no scViews
...
function showDDMenu() {
    if (!(window.matchMedia("all and (min-width: 769px)").matches)) {
        if (document.getElementById("menu").getElementsByTagName("ul")[0].style.width == "") { //set width of the menu ul element so that it shows
            document.getElementById("menu").getElementsByTagName("ul")[0].style.width = "200px";
        } else { //clear menu ul element width so it hides
            document.getElementById("menu").getElementsByTagName("ul")[0].style.width = "";
        }
    }
}
...
function showSearch() {
    ...
    if (document.getElementById("search").style.visibility == "visible") { //clear styles set in JS, forcing fallback to CSS style file
        if (matchesMediaQ) {
            document.getElementById("menu").style.visibility = "";
        } else {
...
function loadNamesJSON() {
    ...
    if (sessionStorage.getItem("wpIndex")) {
        ...
        try { //check to make sure the browser has enough storage
            sessionStorage.setItem("wpIndex", jsonhttp.response)
        } catch (error) { //if the browser doesn't have enough storage we don't store the json file
...
function searchArray(string, data) {
    ...
    if (string == "") { //if no string, output nothing
        results = null;
        //and hide the table
        //hideElement("tableBorder");
        document.getElementById("tableBorder").style.visibility = "";
    } else { //if string, find entries contain said string
        ...
        for (var entryIterator = 0; entryIterator < data.length; entryIterator++) {
            ...
            if (!isNaN(string)) { //if the string can be caste as a number, search the ID field.
                if (thisEntry["i"] == string) {
                    results.push(thisEntry);
                }
            } else { //new method comparing 1-1, 1-2, 1-3, 2-1, 2-2, 2-3, etc.
                ...
                for (var strElementIterator = 0; strElementIterator < stringArray.length; strElementIterator++) {
                    ...
                    if (elementMatches > 0) { //if there are element matches, increase the matches count.
                        matches++;
                    } else if (strElementIterator == 0) { //if we get through the first string element and there are no matches, stop looking.
                        break;
                    }

                }

                if (matches == stringArray.length) { //if we have as many matches as elements of the search string, add this entry.
                    results.push(thisEntry);
                }
            }
        }
    ...
    if (results != null) {
        if (results.length > 0) {
            for (var iterator = 0; iterator < results.length; iterator++) { //build the HTML for the table from the results
